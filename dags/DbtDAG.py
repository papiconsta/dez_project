...
with DAG(
    ...
    dbt_show = DockerOperator(
        task_id='dbt_show_query_3',
        image='dez-dbt:latest',
        command='dbt show -s query_3 --limit 20 --profiles-dir /dbt',
        working_dir='/dbt',
        mounts=[
            Mount(source=DBT_HOST_DIR, target='/dbt', type='bind'),
            Mount(source=SECRETS_HOST_DIR, target='/secrets', read_only=True, type='bind'),
    ...
    trigger_aggregations = TriggerDagRunOperator(
        task_id='trigger_aggregations',
        trigger_dag_id='dbt_aggregations_dag',
        wait_for_completion=True,
        poke_interval=30,
    )

    dbt_run >> dbt_test >> dbt_show >> trigger_aggregations
