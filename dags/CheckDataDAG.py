...
def task1_wrapper():
    log.info("Starting Task 1: Fetching data from GCS for cleaning.")
...
def task2_wrapper():
    log.info("Starting Task 2: Data Janitor will clean now your data...")
    import os
    directory = "/tmp/data/"
...
def task3_wrapper():
    log.info("Starting Task 3: Decontaminationated data are being stored to BigQuery.")
    import os
    directory = "/tmp/data/"
    loadDataToStaging(
        files=[os.path.join(directory, f) for f in os.listdir(directory) if f.endswith(('.parquet', '.csv'))]
...
with DAG(
    ...
    trigger_dbt = TriggerDagRunOperator(
        task_id='trigger_dbt_check',
        trigger_dag_id='dbt_check_dag',
        wait_for_completion=True,
        poke_interval=30,
    )

    task1 >> task2 >> task3 >> trigger_dbt
